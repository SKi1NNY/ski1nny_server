...
def test_login_refresh_and_protected_profile_flow(client: TestClient):
    ...
    assert invalid_refresh_response.json()["error_code"] == "invalid_refresh_token"
...
def test_inactive_and_deleted_users_are_blocked(client: TestClient, db_session):
    ...
    assert inactive_response.json()["error_code"] == "inactive_user"
    ...
    assert deleted_response.json()["error_code"] == "deleted_user"
